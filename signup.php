...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'signup') {
    ...
    if (!preg_match('/^0[0-9]{9}$/', $phone_number)) {
        $error = "Please enter a valid 10-digit South African phone number (e.g., 0821234567).";
        $_SESSION['signup_attempts']++;
    } elseif (empty($username) || empty($email) || empty($password) || empty($phone_number)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, email, password, phone_number, role) VALUES (?, ?, ?, ?, 'user')");
            if ($stmt === false) {
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8">
    <div class="bg-white p-6 sm:p-8 rounded-lg shadow-lg w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg">
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                <label for="phone_number" class="block text-gray-700 text-sm sm:text-base">Phone Number (e.g., 0821234567)</label>
                <input type="tel" id="phone_number" name="phone_number" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm sm:text-base" pattern="0[0-9]{9}" title="Please enter a valid 10-digit South African phone number (e.g., 0821234567)" required>
            </div>
            <div>
            ...
            <div class="mt-6">
                ...
                <form method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="tel" id="recover_phone" name="phone_number" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm sm:text-base" pattern="0[0-9]{9}" title="Please enter a valid 10-digit South African phone number (e.g., 0821234567)" required>
                    </div>
