...
// Fetch featured products (top 3 by product_id)
...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if (!isset($_SESSION['user_id'])) {
        ...
        if ($action === 'view') {
            ...
            $stmt = $conn->prepare("SELECT name, price, description FROM products WHERE product_id = ?");
            $stmt->bind_param("i", $product_id);
            $stmt->execute();
...
if (isset($_SESSION['pending_action']) && isset($_SESSION['pending_product_id']) && isset($_SESSION['user_id'])) {
    ...
    unset($_SESSION['pending_action'], $_SESSION['pending_product_id']);
...
<body>
    <div class="flex">
        ...
        <div class="flex-1 sm:ml-64 p-6 bg-gray-50 min-h-screen">
            <div class="bg-blue-600 text-white rounded-xl shadow-lg p-8 mb-8 text-center">
                <h1 class="text-4xl md:text-5xl font-extrabold mb-4">Ubuntu Technologies: Empowering Local Trade</h1>
                <p class="text-lg md:text-xl font-light mb-6">South Africa's Premier <span class="font-semibold">C2C Marketplace</span> – Connect, Buy, and Sell Locally.</p>
                <div class="flex justify-center space-x-4">
                    <a href="/Main/products.php" class="inline-block bg-white text-blue-600 hover:bg-gray-100 font-bold py-3 px-8 rounded-full shadow-md transition-all duration-300 transform hover:scale-105">Explore Products Now!</a>
                    <a href="/Main/listing.php" class="inline-block bg-white text-blue-600 hover:bg-gray-100 font-bold py-3 px-8 rounded-full shadow-md transition-all duration-300 transform hover:scale-105">Create Listing</a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div class="bg-white p-6 rounded-lg shadow-md">
                    ...
                    <p class="text-gray-700 leading-relaxed">Ubuntu Technologies offers a secure online marketplace for South Africans to buy and sell goods directly. From local crafts to electronics, our platform connects individuals, fostering a thriving C2C community with easy listing and purchasing options.</p>
                </div>
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <h2 class="text-2xl font-semibold text-gray-800 mb-4 flex items-center">
                        <i class="fas fa-bullseye text-blue-600 text-3xl mr-3"></i> Our Mission
                    </h2>
                    <p class="text-gray-700 leading-relaxed">We aim to empower local trade by connecting South African communities. Through trust, transparency, and accessibility, we support entrepreneurs and buyers, enhancing economic growth across the nation.</p>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow-md mb-8 text-center">
                ...
                <p class="text-gray-600 mb-6">Based on thousands of satisfied customers across South Africa.</p>
    ...
    <div id="productModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-details">
                ...
                <form id="addToCartForm" method="POST" action="cart.php" class="flex items-center">
    ...
    <script>
        ...
        document.getElementById('addToCartForm').addEventListener('submit', function(e) {
            ...
            fetch('/Main/home.php?action=add&id=' + product_id + '&quantity=' + quantity, {
                method: 'GET'
            })
