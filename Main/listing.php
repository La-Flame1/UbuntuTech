...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_listing'])) {
    ...
    $layout_option = trim($_POST['layout_option'] ?? 'Standard');
    $serial_number = trim($_POST['serial_number'] ?? '');

    if (empty($name) || empty($description) || $price <= 0 || $stock < 0) {
        $error = "All fields (name, description, price, stock) are required, and price/stock must be valid.";
    } else {
        $stmt = $conn->prepare("INSERT INTO products (name, description, price, stock, image_path, catalog, layout_option, serial_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssdsssss", $name, $description, $price, $stock, $image_path, $catalog, $layout_option, $serial_number);
        if ($stmt->execute()) {
...
$stmt = $conn->prepare("SELECT product_id, name, price, image_path FROM products WHERE product_id IN (SELECT product_id FROM products WHERE user_email = ?)");
...
<div class="sm:ml-64 p-6 flex-1">
    ...
    <div class="bg-white p-6 rounded-lg shadow mb-8">
        ...
        <form method="POST" action="listing.php" class="space-y-4">
            ...
            <div>
                <label for="layout_option" class="block text-gray-700 font-medium mb-1">Layout Option</label>
                <input type="text" id="layout_option" name="layout_option" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="Standard">
            </div>
            <div>
                <label for="serial_number" class="block text-gray-700 font-medium mb-1">Serial Number</label>
                <input type="text" id="serial_number" name="serial_number" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
            </div>
